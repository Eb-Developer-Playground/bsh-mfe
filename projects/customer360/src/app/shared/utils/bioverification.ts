...
type ActionTicketValidationMethod = 'validateMandateDocuments' | 'validateMandateBioVerify';
...
export class BioVerificationUtil {
  ...
  static launchBio(config: BioVerificationConfig): Observable<any> {
    ...
    if (!isCustomerPresent && actionTicketsService && ticketId && actionFlow) {
      ...
      return actionTicketsService.sendNotPresentBio(ticketId, actionFlow, notPresentPayload)
        .pipe(
          switchMap(response => {
            if (response?.successful) {
              return actionTicketsService.validateMandateBioVerify(ticketId, actionFlow);
            }
            return of({ successful: false, statusMessage: 'Failed to send not present bio' });
          }),
          switchMap(response => {
            if (response.successful) {
    ...
    return of({ successful: false, statusMessage: 'Invalid configuration for bio verification' });
  }

  /**
   * Opens the signatories dialog for verification
   */
  private static openSignatoriesDialog(config: BioVerificationConfig, account: any): Observable<any> {
    const {
      dialog,
      customerData,
      actionTicketsService,
    ...
    return new Observable(observer => {
      dialogRef.afterClosed().subscribe(result => {
        if (result && result.status === 'canVerify' && result.data) {
          this.openVerifySignatoryBioDialog(config, result.data, account)
            .subscribe(
  ...
  private static openVerifySignatoryBioDialog(
    ...
    return new Observable(observer => {
      dialogRef.afterClosed().subscribe(async result => {
        if (result && result.data && actionTicketsService && ticketId && actionFlow) {
          try {
            const validationResponse = await firstValueFrom( actionTicketsService
              .validateMandateBioVerify(ticketId, actionFlow)
            );
            ...
            if (validationResponse.successful) {
              if (toastService && translateService) {
                toastService.show(
                  'Success',
                  translateService.instant('CUSTOMER.ACCOUNT-SERVICES.CHANGE-OF-SIGNATORY.ADDED-SUCCESS'),
                  MessageBoxType.SUCCESS,
                  5000,
              ...
              throw new Error(validationResponse.statusMessage || 'Validation failed');
            }
          } catch (error) {
            if (toastService && translateService) {
              toastService.show(
                'Error',
                translateService.instant('CUSTOMER.ACCOUNT-SERVICES.CHANGE-OF-SIGNATORY.ADDED-FAIL'),
                MessageBoxType.DANGER,
                5000,
                undefined, undefined, false
              );
   ...
   */
  private static openVerifyBioDialog(config: BioVerificationConfig, account: any): Observable<any> {
    const {
      dialog,
      customerData,
      actionTicketsService,
        ...
        if (result && result.data && actionTicketsService && ticketId && actionFlow) {
          try {
            ...
            //   : 'validateMandateBioVerify';
              const validationMethod: ActionTicketValidationMethod =
              actionFlow.toLowerCase().includes('mandate')
                ? 'validateMandateDocuments'
                : 'validateMandateBioVerify';
            const validationResponse = await (actionTicketsService[validationMethod]as Function)(
                ticketId,
                actionFlow
              ).toPromise();
