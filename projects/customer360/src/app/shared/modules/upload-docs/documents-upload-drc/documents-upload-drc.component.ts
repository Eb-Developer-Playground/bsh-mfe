  ...
  implements OnInit, OnChanges, OnDestroy
{
  ...
  private initializeCloneOfObjects(): void {
    ...
    this.cloneOfObjects = this.documentSpecs.map((doc: any) => {
      ...
      const fileName = reusedData
        ? `reused_${doc.name.toLowerCase()}.jpeg`
        : '';
      ...
      return {
        ...
        document: reusedData?.data
          ? {
              filename: `reused_${doc.name.toLowerCase()}.jpeg`,
  ...
  async handleCapturedImage(
    ...
    const fileType = isScanned ? file.format : file.type.split('/')[1];
  ...
  async onChange(event: any, object?: IUploadedDocument) {
    ...
    if (object) {
      ...
      if (doc?.fileTypes) {
        const fileExtension = _file.type.split('/')[1];
        if (
      ...
      object.document = {
        filename: object.key || object.name,
