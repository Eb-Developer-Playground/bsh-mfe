...
import { CountryCode, Feature, FeatureFlagsConfig, FEATURE_FLOWS, FeatureFlowName } from './feature-flags.model';
...
export class FeatureFlagService {
  ...
  getConfiguredFlow<F extends Feature>(country: CountryCode, feature: F): FeatureFlowName<F> | undefined {
    const flow = this.cfg.actionFlowNames?.[country]?.[feature] as FeatureFlowName<F> | undefined;
    ...
    if (!allowed.includes(flow)) {
      if (isDevMode()) {
        console.error(`[FeatureFlags] Invalid flow "${flow}" for ${feature} in ${country}. Allowed: ${allowed.join(', ')}`);
      }
      return undefined;
    }
    return flow;
   ...
   */
  isActionFlowNameEnabled<F extends Feature>(country: CountryCode, feature: F, flow: FeatureFlowName<F>): boolean {
   ...
   */
  assertFlowEnabled<F extends Feature>(country: CountryCode, feature: F, flow: FeatureFlowName<F>): boolean {
