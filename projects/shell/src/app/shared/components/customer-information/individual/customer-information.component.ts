...
export class CustomerInformationComponent implements OnInit {
  ...
  hasDigitalSupportRole = signal<boolean>(false);
  ...
  constructor(
    ...
    protected translate: TranslateService
      ...
      this.hasDigitalSupportRole.set(sessionService.hasRole("SoftDelete.Maker"))
  }
  ...
  ngOnInit(): void {
    ...
    this.maritalStatus = this.cifInquiryObj?.personalDetails?.maritalStatus
      ? (new MaritalStatusPipe().transform(
          this.cifInquiryObj?.personalDetails?.maritalStatus
        ) as string)
      : '';

    this.customerDetails.personalDetails = this.cifInquiryObj.personalDetails;
    this.customerDetails.identifications =
      this.cifInquiryObj.identificationDetails;

    this.canDisplayImage = this.sessionService.hasFeatureRole(
      'AccountManagement.ViewSignatureAndPhoto'
    );
    let channelsList: any = localStorage.getItem(<string>'channels');
  ...
  getCountry(code: string) {
    return (
      this.countries?.find((el: any) => el.countryCode === code)?.countryName ??
      code
    );
  }

  getNationality = (code: string) => {
    return (
      this.nationalities?.find((el: any) => el.nationalityCode === code)
        ?.nationalityName ?? code
    );
  };

  getTitles = (code: string) => {
    return this.titles?.find((el: any) => el.value === code)?.titleDesc ?? code;
  };
  ...
  getRegion = (code: string) => {
    return (
      this.regions?.find(
        (el: any) =>
          el.value === code &&
          Number(this.sessionService.userBank) === el.bank_Id
      )?.regionLocation ?? code
    );
  };
  ...
  getCifInquiry = () => {
    ...
    this.accountService['cifInquiryV2'](this.isBusiness, payload).subscribe(
      (res: any) => {
        ...
        if (this.isBusiness) {
          this.docDesc = cifInquiryObj?.companyDetails?.docDesc;
          this.refNum = cifInquiryObj?.companyDetails.registrationNumber;
        } else {
          const dits = this.cifInquiryObj?.identificationDetails.find(
            (dits: any) => dits.referenceNum !== ''
          );
          this.docDesc = dits.docDesc;
          this.refNum = dits.referenceNum;
        }

        if (
          !this.kraPin &&
          ['KE'].includes(this.sessionService.subsidiary.countryCode)
        ) {
          this.kraPinEmptyStaticDataRequired();
        }
      },
      (err: any) => {
        this.logAudit(
            ...
            this.channelsService['getCustomerChannels'](this.bankId, this.preferredPhoneNumber)
              .subscribe((res: ChannelsResponse) => {
                if (res.statusCode !== '99') {
                  ...
                  this.toast.show(
                    'CUSTOMER.CUSTOMER-PROFILE.UNABLE-TO-VERIFY-CID',
                    '',
  ...
  updateProfile() {
    ...
    const bankId = this.sessionService.subsidiary.bankId;
    const dormantAccExists = <string>localStorage.getItem('dormantAccExists');
    if (dormantAccExists === 'true' && ['56', '43', '11'].includes(bankId)) {
      this.toast.show(
        null,
    ...
    const country = this.sessionService.subsidiary.countryCode.toLowerCase();
    url.pathname = `/services/static-data/${country}/${customer}`;
  ...
  goToSearch() {
    if (this.customerStatus === 'InActive') {
      ...
      this.router.navigateByUrl('/services/omnichannel-profile').then(r => {});
    }
  }
      ...
      (res: any) => {
        if (res.successful && res.responseObject.length > 0) {
          ...
          const taskData = JSON.parse(
            this.blockedInformation.taskData || this.blockedInformation.TaskData
          );
          this.profileBlockedInformation.blockedBy =
            this.blockedInformation.createdBy ||
            this.blockedInformation.CreatedBy;
          this.profileBlockedInformation.date =
            this.blockedInformation.createdOnUtc ||
            this.blockedInformation.CreatedOnUtc;
          this.profileBlockedInformation.reason = taskData.Comment;
  ...
  kraPinEmptyStaticDataRequired() {
    const dialogRef = this.dialog.open(DialogConfirmComponent, {
      width: '400px',
      //height: '180px',
      disableClose: true,
      data: {
        title: 'TOAST.STATIC-DATA-REQUIRED',
        // pillText: "KNOWN-AGENT.INVALID-DETAILS",
        // bodyDescription: `${this.translateService.instant('KNOWN-AGENT.CANNOT-BE-VERIFIED') }`,
        bodyDescription: 'TOAST.CUSTOMER-DOES-NOT-HAVE-KRA-NUMBER',
        showCloseIcon: false,
        cancelButtonLabel: 'COMMON.CANCEL',
        confirmButtonLabel: 'COMMON.PROCEED-STATIC-DATA',
      },
    });
    ...
    dialogRef.afterClosed().subscribe((option: any) => {
      if (option?.confirm) {
        ...
        //   idNumber: string;
        // } = {
        //   idNumber: this.cifInquiryObj.personalDetails.idNumber,
        //   cif:this.cifInquiryObj.personalDetails.customerId
        // }
        this.updateProfile();
      }
    });
