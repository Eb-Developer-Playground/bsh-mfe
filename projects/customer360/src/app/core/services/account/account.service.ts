...
export class AccountService {
  ...
  private specialRates$ = new BehaviorSubject<any[]>([]);
  ...
  public getAllMISCodes(): Observable<any[]> {
    return this.api
      .get<{ responseObject: any[] }>('/v1/backoffice/onboarding/freeCodes')
      .pipe(map(response => response.responseObject || []));
  }

  public getMISCodesByType(
    type: 'FREE_CODE_3' | 'FREE_CODE_5' | 'FREE_CODE_7' | 'FREE_CODE_8'
  ): Observable<any[]> {
    return this.api
      .get<{ responseObject: any[] }>(
        `/v1/backoffice/onboarding/freeCodesByType/${type}`
      )
      .pipe(map(response => response.responseObject || []));
  }
  ...
  public getStakeHolders(accountNumber: string, bankId: string): Observable<any> {
    return this.api.get<any>(
