...
@Component({
  ...
  templateUrl: './otp-codes.component.html',
  styleUrls: ['./otp-codes.component.scss'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: OtpCodesComponent,
      multi: true,
    },
    {
...
export class OtpCodesComponent implements ControlValueAccessor {
  @Input()
  ...
  writeValue(value: string): void {
    ...
    if (regex.test(value)) {
      for (let i = 0; i < value.length; i++) {
        this.inputs.controls[i].setValue(value[i]);
      }
    } else {
  ...
  selectInput(idx: number): void {
    this.inputEls.get(idx)?.nativeElement.focus();
  }
