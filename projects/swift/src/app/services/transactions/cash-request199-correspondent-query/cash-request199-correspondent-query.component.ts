...
@Component({
  ...
  selector: 'app-cash-request199-correspondent-query',
  templateUrl: './cash-request199-correspondent-query.component.html',
  styleUrls: ['./cash-request199-correspondent-query.component.scss'],
})
export class CashRequest199CorrespondentQueryComponent implements OnInit, AfterViewInit, OnDestroy {
  fieldsForm!: UntypedFormGroup;
  actionForm!: UntypedFormGroup;
  formFields: any[] = FIELDS[PROCESSES.CORRESPONDENT_QUERY];
  ...
  ngAfterViewInit() {
    ...
    this.fieldsForm.patchValue({
      ...
      NewValue: taskData?.NewValue,
      RecieverBIC: taskData?.RecieverBIC,
      RecallCancelationType: this.isReturnTicket
        ? taskData?.RecallCancelationType
  ...
  submitTicket(ticketId: any): void {
    this.transService.submitTicket(ticketId, { ...this.fieldsForm.value }, 'mt199').subscribe(
      res => {
        if (res.successful && res.responseObject) {
          this.submitted = true;
          this.toast.show(
